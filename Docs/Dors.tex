...
As well as these {\it FORTRAN} programs, ``The-Dors" requires various mathematical programs which we shall combine into a library. The good news is
that they are all free, copyright free, and patent free. You can distribute freely (as long as you don't say you wrote
 them and so on. If you do distribute them - say where they are from!  However, I have chosen {\it not} to distribute
them with ``The-DORS" (The reason is I don't have the space in my {\it github} account to distribute
them from there.  Instead, ``The-DORS" file contains a subdirectory called  {\it dorslibs}.
...
That is, it tells you all the maths library routines you need to download, and where to download them from. 
(I may put them all on {\it zSHARE} so you can download them all as a gzipped tarfile. They take up
...
Note that this library will contain the {\it HITRAN} subroutines and functions that you downloaded, and is linked
 with {\it -lDors} at the compilation stage.
