...
We are prompted as to whether we want this output or not. For now, enter zero.
...
to 1$\mu m$ radiation, and so on. So, if we denote the wave number as $\nu (cm^{-1})$ and the wavelength as $\lambda (\mu m)$
then $\nu=10^4/\lambda$. We make the trivial observation that $d \nu/\nu=-d \lambda / \lambda$.
...
We are prompted for these in turn. For now use $5 cm^{-1}$ for the cut-off, and $0.05 cm^-1$ for the step size.
...
temperature, then the temperature at the ground ($T[0]$ is changed to $groundTemp$, and the
temperature at 1km ($T[1]$ is set to $[T[1]*groundTemp/T[0]$  --- all other temperatures are unchanged. For now, just
...
aeroplane remote sensing you are prompted for a height. Select zero when prompted as to whether you want to do
aeroplane remote sensing for now.
...
files and not use the results of any previous calculation. The 10 on the next line tells us that we are using
10 point Gauss quadrature for the integration over the cumulative-$\kappa$ distribution for calculating non-grey 
radiative transfer. The next line ends the wavelength related inputs, and tell us we have a cut-off of $5 cm^{-1}$
 and a step size of $0.05 cm^{-1}$.
...
Nevertheless, we must include an aeroplane  height on this line.  Again we have made it a large negative
