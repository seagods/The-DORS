...
The best thing to do when first using {\it Dors1} (assuming you are in your {The-DORS} directory) is to just type ``Dors1 -v [ENTER]" at a system terminal.
...
We shall need to consider absorption lines outside the wavelength regions specified, we are asked for a 
$\Delta \lambda$ (in microns) so we will consider lines from $\lambda_1-\Delta \lambda$ to $\lambda_2$. To
a first approximation the wave numbers will vary from $\tilde{\nu_1}(1-\Delta \lambda/\lambda_2)$
...
For this reason the next input decides whether to calculate molecular absorption spectra or not.
...
We may not wish to re-do these calculations and over-write the previously
 calculated file: if so  
we should enter zero.
...
These data files shall be written to the {\it MolSpect} directory. Note that if you want to keep these files, you must create
...
You quite probably have software to plot the spectra, if not you might choose to
 get {\it PlotIt} which is one of my own graphics programs \footnote[1]{git@github.com:seagods/PlotFiles.git}.
...
If we choose a cut-off integer $\rm{i}_{CD}$ so that the cut-off for a pure Doppler line is $ \rm{i}_{CD} \alpha_D$
then the Doppler broadened line falls off as $\exp(-[\rm{i}_{CD}]^2)$. So, 
for $\rm{ i}_{CD}=5$, the value of the line shape at the
cut-off is roughly 0.7\% of the value at the line centre. 
If we choose a cut-off integer $\rm{ i}_{CL}$ so that the cut-off for a pure Lorenz line is $ \rm{i}_{CL} \alpha_L$
then the Lorentz line falls off as $1/(1+\rm{ i}_{CL}^2$). So, for $\rm{ i}_{CL}=12$, the value of the line shape at the
cut-off is also roughly 0.7\% of the value at the line centre. If it is required that more of the line is within
the cut-off, then we need to increase ${\rm i}_{CL}$ more than ${\rm i}_{CD}$ since the Doppler profile falls off much more rapidly
than the Lorentz profile. The actual value of the cut-off is to taken as whichever value of ${\rm i}_{CD} \alpha_D$
 and ${\rm i}_{CL} \alpha_{DL}$ is largest. We need another integer for the spectral resolution. When calculating the spectra
