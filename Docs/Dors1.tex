...
The following descriptions of the various options are rather turgid, but will be handy to have alongside
...
problems, and then combined at the end. (No real instrument has a truly triangular form, putting in
 a few extra points 
around the sharply curved peak of a response function that is almost triangular is a good idea.)

 There must be at least two wavelengths between $\lambda_1$ and $\lambda_2$.
...
Atmosphere numbers 1, 6, and 7 are tropical, standard, and user defined and are independent of season.
...
The first is the usuall Gauss-Legendre rule that calculates integrations in height in each sub-layer of each
...
of the absorption lines.
 these are two more another things that you need to experment with. For now, just choose 4 and 16.
...
to enter visibilities and chosen a tropospheric visibility of 45km and a boundary layer visibilty
...
When run in verbose mode, there is no input number for the tropospherical aerosol type. 
...
The next line indicates that we have chosen 1 and shall use the default water vapour profiles, and 0 
 to indicate we have no information  on either temperature or pressure. 
