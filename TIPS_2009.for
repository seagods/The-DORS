C***********************
      Program TIPS_2008
C*********************** 
C    Program TIPS_2008 written by R.R. Gamache
C 
c
c...date last changed 15 May, 2008
c  --  UPDATES  --
C     5 May, 2008 better high temperature CH4 partition sums, see the paper
c     Ch. Wenger, J.P. Champion, V. Boudon, "The partition sum of methane 
c     at high temperature," J Quant Spectros Radiat Transfer 109,
c     Issue 16, November 2008, Pages 2697-2706
c
...
c
    5 WRITE (*,'(8x,A,/,8x,A////////////)')
     + '   This program calculates the Total Internal Partition Sum'
     + ,'       for the molecular species on the HITRAN database.'
c
   10 continue
C
C
      WRITE (*,'(///A//)')
     + '   ENTER Molecule number, the choices are:'
       WRITE (*,'(6(I4,''= '',A6)/)') (J,MOLID(J),J=1,NMOL)
       WRITE (*,'(//A//)')
     +'     99 ends the list'
   20  READ (*,*,err=21) MOL
       go to 22
   21 write(*,'(a)') '   Incorrect DATA type, try again'
      go to 20
C
...
c
C
       CALL ISO_82_TO_85 (MOL,NSO82,ISO)
C
c
   30  WRITE (*,'(//A,/,A/)')
     +'    Enter Temperature for calculation of Q(T)',
     +'    Current range is 70 - 3000 K'
       READ (*,*,err=31) Temp
       go to 32
   31 write(*,'(a)') '   Incorrect DATA type, try again'
      go to 30
c
...
c
 100  if(QT .gt. 0.) then
        Write(*,900) MOLID(Mol), NSO82, Temp, QT, gi
  900 format(//,5x,A6,' Molecule',3x,i5,' Isotopic species',/,
     + 5x,'Q(',f7.1,' K) = ',1pE12.6,5x,'State Independent factor=',
     + 0pf7.1,////)
      endif
      
      go to 10
  99  STOP'  Computation finished'
      END 
...
C
   ...
   10 CONTINUE
      ...
      include 'BD_MOL.FOR'
      include 'BD_ISO_2002.FOR'
      include 'BD_ISO_82_TO_85.FOR'
...
c...      CH4
c...        --       211
c...J-P Chamapion calculations JQSRT
